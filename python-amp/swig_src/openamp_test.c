...
{
	int bytes_rcvd_pos, bytes_sent_pos;
	int err_cnt_pos = 0;
	int X_pos_rec = 0, Y_pos_rec = 0;
	i_payload->num = 0;
	i_payload->size = 8;
	i_payload->data[0] = (unsigned char)((X_pos >> 24) & 0x000000ff);
	i_payload->data[1] = (unsigned char)((X_pos >> 16) & 0x000000ff);
	i_payload->data[2] = (unsigned char)((X_pos >> 8) & 0x000000ff);
	i_payload->data[3] = (unsigned char)((X_pos) & 0x000000ff);
	i_payload->data[4] = (unsigned char)((Y_pos >> 24) & 0x000000ff);
	i_payload->data[5] = (unsigned char)((Y_pos >> 16) & 0x000000ff);
	i_payload->data[6] = (unsigned char)((Y_pos >> 8) & 0x000000ff);
	i_payload->data[7] = (unsigned char)((Y_pos) & 0x000000ff);
	bytes_sent_pos = write(fd, i_payload,(2 * sizeof(unsigned long)) + 8);
	//r_payload->num = 0;
	//bytes_rcvd_pos = read(fd, r_payload, (2 * sizeof(unsigned long)) + PAYLOAD_MAX_SIZE);
	//while (bytes_rcvd_pos <= 0) {
	//	usleep(10000);
	//	bytes_rcvd_pos = read(fd, r_payload, (2 * sizeof(unsigned long)) + PAYLOAD_MAX_SIZE);
	//}
	//int k;
	//for (k = 0; k < r_payload->size; k++) {
	//	if (r_payload->data[k] != i_payload->data[k]) {
	//		//printf(" \r\n Data corruption");
	//		//printf(" at index %d \r\n", k);
	//		err_cnt_pos++;
	//		break;
	//	}
	//}
	/*X_pos_rec = (int)((((unsigned int)(r_payload->data[0])) << 24) + (((unsigned int)(r_payload->data[1])) << 16) +
		(((unsigned int)(r_payload->data[2])) << 8) + ((unsigned int)(r_payload->data[3])));
	Y_pos_rec = (int)((((unsigned int)(r_payload->data[4])) << 24) + (((unsigned int)(r_payload->data[5])) << 16) +
		(((unsigned int)(r_payload->data[6])) << 8) + ((unsigned int)(r_payload->data[7])));*/
	//bytes_rcvd_pos = read(fd, r_payload, (2 * sizeof(unsigned long)) + PAYLOAD_MAX_SIZE);
	//printf("\r\n Test Results: Error count = %d\r\n", err_cnt_pos);
	/*printf("\r\n X_pos:%d\r\n", X_pos_rec);
	printf("\r\n Y_pos:%d\r\n", Y_pos_rec);
	if (err_cnt_pos == 0)
		return (X_pos_rec+ Y_pos_rec);
	else*/
		return 0;
}
